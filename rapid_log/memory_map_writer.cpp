...
bool CMemoryMapWriter::impl::i_OpenFile(const TCHAR *path, bool cover_write)
{
	...
	if (!handle_file_ || handle_file_ == INVALID_HANDLE_VALUE) {
		...
		handle_file_ = 0;
		assert(false);
		throw Win32Error("CreateFile", error);
		return false;
	}
...
bool CMemoryMapWriter::impl::i_CreateFileMap()
{
	...
	if (file_size_ == 0xFFFFFFFF) {
		Close(); // 最大只写4G
		assert(false);
		throw Win32Error("max size limit", 0);
		return false;
	}
	...
	if (handle_map_ == INVALID_HANDLE_VALUE) {
		...
		Close();
		assert(false);
		throw Win32Error("CreateFileMapping", error);
		return false;
	}
	...
	if (!cur_block_ptr_) {
		...
		Close();
		assert(false);
		throw Win32Error("MapViewOfFile", error);
		return false;
	}

	memset(cur_block_ptr_, 0, cur_block_size_);
	return true;
}
